...
export const CategoryManager: React.FC = () => {
  ...
  return (
    <div className="space-y-3">
        ...
        {categories.map((category, index) => (
          <React.Fragment key={category.id}>
            {editingId === category.id ? (
              ...
              <div className="flex items-center gap-2 px-4 py-3" style={{ minHeight: '44px' }}>
                <input
                  ...
                  className="flex-1 bg-[#2C2C2E] md:bg-[#F5F5F7] text-white md:text-[#000000] px-3 py-2 rounded-[8px] text-[15px] tracking-[-0.24px] outline-none"
                  autoFocus
                  onKeyDown={(e) => {
              ...
              <div className="flex items-center gap-3 px-4 py-3" style={{ minHeight: '44px' }}>
                <span className="flex-1 text-[17px] text-white md:text-[#000000] tracking-[-0.41px]">
                  {category.name}
                </span>
                <button
                  onClick={() => handleStartEdit(category.id!, category.name)}
                  className="w-8 h-8 rounded-full bg-[#2C2C2E] md:bg-[#E5E5EA] flex items-center justify-center active:opacity-60 transition-opacity"
                >
            ...
            {index < categories.length - 1 && (
              <div className="h-[0.5px] bg-[#38383A] md:bg-[#C6C6C8] ml-4" />
            )}
          </React.Fragment>
        ))}
      </div>
      ...
      {/* Add New Category */}
      {isAdding ? (
        <div className="bg-[#1C1C1E] md:bg-white rounded-[10px] p-4">
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-[#2C2C2E] md:bg-[#F5F5F7] text-white md:text-[#000000] px-3 py-2 rounded-[8px] text-[15px] tracking-[-0.24px] outline-none placeholder-[#8E8E93]"
              autoFocus
              onKeyDown={(e) => {
        ...
        <button
          ...
          className="w-full bg-[#1C1C1E] md:bg-white rounded-[10px] px-4 py-3 flex items-center gap-3 active:bg-[#2C2C2E] md:active:bg-[#E5E5EA] transition-colors"
        ...
        >
          ...
          <span className="flex-1 text-left text-[17px] text-white md:text-[#000000] tracking-[-0.41px]">
            Add Category
          </span>
        </button>
      )}
