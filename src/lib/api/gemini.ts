...
import { TransactionRecord, dbHelpers } from "../db";
...
const getApiKey = async () => {
  ...
  apiKeyCache = await dbHelpers.getSetting('xpense-api-key') || '';
  return apiKeyCache;
};
...
function getCacheKey(transactions: TransactionRecord[]): string {
  return transactions.map(t => `${t.id}-${t.amount}`).join('|');
}

function getCachedInsights(key: string): string | null {
...
function setCachedInsights(key: string, data: string): void {
...
export const generateInsights = async (transactions: TransactionRecord[]): Promise<string> => {
  const apiKey = await getApiKey();
...
Focus on practical advice that the user can implement immediately.`;
      ...
      const result = response.text || "Could not generate insights.";
