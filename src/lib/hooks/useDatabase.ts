...
import { db, dbHelpers, TransactionRecord, initializeDatabase } from '../db';
...
export function useRecurringExpenses() {
  ...
  const addRecurringExpense = useCallback(async (expense: Omit<import('../db').RecurringExpenseRecord, 'id' | 'createdAt' | 'updatedAt'>) => {
    return await dbHelpers.addRecurringExpense(expense);
  }, []);

  const updateRecurringExpense = useCallback(async (id: number, updates: Partial<import('../db').RecurringExpenseRecord>) => {
